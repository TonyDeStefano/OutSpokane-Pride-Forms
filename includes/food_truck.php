<?php if ( isset( $_GET['txid'] ) ) { ?>
	...
	$entry = new \OutSpokane\FoodTruck;
	$timestamp = '';
	...
	if ( count( $parts ) == 2 )
	{
		...
		{
			$timestamp = $parts[0];
			$entry = new \OutSpokane\FoodTruck( $parts[1] );
		}
	}
						...
						<?php foreach ($entry->getPaymentMethods() as $payment_method_id => $payment_method) { ?>
							<?php if ($payment_method_id == \OutSpokane\Entry::PAYMENT_METHOD_CARD && strlen($stripe_keys['pub']) > 0 && strlen($stripe_keys['secret']) > 0) { ?>
								...
								<form method="post">
									...
									<input type="hidden" name="form" value="food_truck">
									...
									<script
										...
										data-description="<?php echo $entry->getEntryYear(); ?> Food Truck"
...
<?php } else { ?>
	...
	<?php $disable_food_truck_form = get_option( 'pride_forms_disable_food_truck_form', 'N' ); ?>

	<?php if ( $disable_food_truck_form == 'Y' ) { ?>
	...
	<?php } else { ?>
		...
		<div
			...
			<div id="pride-form-step-1">
				...
				<?php \OutSpokane\Entry::drawDefaultFormFields(null, true, ['Organization' => 'Food Truck Name']); ?>
