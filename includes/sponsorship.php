<?php if ( isset( $_GET['txid'] ) ) { ?>
	...
	$entry = new \OutSpokane\Sponsorship;
	$timestamp = '';
	...
	if ( count( $parts ) == 2 )
	{
		...
		{
			$timestamp = $parts[0];
			$entry = new \OutSpokane\Sponsorship( $parts[1] );
		}
	}
						...
						<?php foreach ($entry->getPaymentMethods() as $payment_method_id => $payment_method) { ?>
							<?php if ($payment_method_id == \OutSpokane\Entry::PAYMENT_METHOD_CARD && strlen($stripe_keys['pub']) > 0 && strlen($stripe_keys['secret']) > 0) { ?>
								...
								<form method="post">
									...
									<input type="hidden" name="form" value="sponsorship">
									...
									<script
										...
										data-description="<?php echo $entry->getEntryYear(); ?> Sponsorship"
...
<?php } else { ?>
	...
	<?php $disable_sponsorship_form = get_option( 'pride_forms_disable_sponsorship_form', 'N' ); ?>

	<?php if ( $disable_sponsorship_form == 'Y' ) { ?>

		<div class="alert alert-info">
			Sponsorships are currently offline.
		</div>
	...
	<?php } else { ?>
		...
		<div
			...
			<div id="pride-form-step-1">
				...
				<?php \OutSpokane\Entry::drawDefaultFormFields( NULL, FALSE ); ?>
				...
				\OutSpokane\Entry::drawFormField(
					'Local Contact First Name',
					'local_first_name'
				);

				\OutSpokane\Entry::drawFormField(
					'Local Contact Last Name',
					'local_last_name'
				);
