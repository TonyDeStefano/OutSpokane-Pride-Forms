<?php if ( isset( $_GET['txid'] ) ) { ?>
	...
	$entry = new \OutSpokane\BowlingEntry();
	$timestamp = '';
	...
	if ( count( $parts ) == 2 )
	{
		...
		{
			$timestamp = $parts[0];
			$entry = new \OutSpokane\BowlingEntry( $parts[1] );
		}
	}
							...
							<?php if ($payment_method_id == \OutSpokane\Entry::PAYMENT_METHOD_CARD && strlen($stripe_keys['pub']) > 0 && strlen($stripe_keys['secret']) > 0) { ?>
								...
								<p>
									<form method="post">
										...
										<input type="hidden" name="form" value="bowling">
...
<?php } else { ?>
	...
	<?php $disable_bowling_form = get_option( 'pride_forms_disable_bowling_form', 'N' ); ?>

	<?php if ( $disable_bowling_form == 'Y' ) { ?>

		<div class="alert alert-info">
			Bowling entries are no longer available for purchase.
		</div>
	...
	<?php } else { ?>
		...
		<div
			...
			<div id="pride-form-step-1">
				...
				<?php \OutSpokane\Entry::drawDefaultFormFields( array( 'organization' ), TRUE ); ?>
                            ...
                            <optgroup label="Individual Tickets">
                                <?php for ( $x = 1; $x <= 3; $x++ ) { ?>
                                    <option value="<?php echo $x; ?>">
                                        <?php echo $x; ?> - $<?php echo number_format( \OutSpokane\BowlingEntry::PRICE_PER_TICKET * $x, 2 ); ?>
                                    </option>
                                <?php } ?>
                            </optgroup>
                            <optgroup label="Team Tickets">
		                        <?php for ( $x = 4; $x <= 5; $x++ ) { ?>
                                    <option value="<?php echo $x; ?>">
				                        Team of <?php echo $x; ?> - $<?php echo number_format( \OutSpokane\BowlingEntry::PRICE_PER_TICKET * $x, 2 ); ?>
                                    </option>
		                        <?php } ?>
                            </optgroup>
                        </select>
