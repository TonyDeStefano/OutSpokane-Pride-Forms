...
<div class="wrap">
	...
	<?php if ( $action == 'view' ) { ?>
		...
		<?php $entry = new \OutSpokane\BowlingEntry( (isset($_GET['id'])) ? $_GET['id'] : 0 ); ?>
		...
		<?php } else { ?>
			...
			<h1>
				...
				<a href="?page=outspokane_edit_entry&form=bowling&id=<?php echo $entry->getId(); ?>" class="page-title-action">
					Edit
				</a>
				<a href="#" class="page-title-action" id="pride-delete-entry" data-form="bowling" data-id="<?php echo $entry->getId(); ?>">
					Delete
				</a>
			</h1>
			...
			<div class="row">
				<div class="col-md-7">
					...
					<div class="well" id="pride-forms-update-details" data-form="bowling" data-id="<?php echo $entry->getId(); ?>">
						...
						<table class="table">
							...
							<tr>
								<th>Event:</th>
								<td><?php echo $entry->getEntryYear(); ?> Bowling Ticket</td>
							</tr>
							...
							<tr>
								...
								<td>
                                    <?php if ( $entry->isTeam() ) { ?>
	                                    <?php echo $entry->getQty(); ?> Person Team
                                    <?php } else { ?>
					...
					{
						if ( preg_match('/\[pride_forms[^\]]*form=["\']bowling["\'][^\]]*\]/', $page->post_content) === 1)
						{
				...
				<div class="col-md-5">
					...
					<div class="well" id="pride-forms-update-entry-notes" data-form="bowling" data-id="<?php echo $entry->getId(); ?>">
					...
					<div class="well" id="pride-forms-mark-as-paid" data-form="bowling" data-id="<?php echo $entry->getId(); ?>">
	...
	<?php } else { ?>
		...
		\OutSpokane\Entry::drawExportForm( \OutSpokane\BowlingEntry::TABLE_NAME );
		$table = new \OutSpokane\EntryTable( \OutSpokane\BowlingEntry::TABLE_NAME );
		$table->prepare_items();
		$table->display();
