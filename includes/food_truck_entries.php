...
<div class="wrap">
	...
	<?php if ( $action == 'view' ) { ?>
		...
		<?php $entry = new \OutSpokane\FoodTruck( (isset($_GET['id'])) ? $_GET['id'] : 0 ); ?>
		...
		<?php } else { ?>
			...
			<h1>
				View Food Truck Entry
				<a href="?page=<?php echo $_REQUEST['page']; ?>" class="page-title-action">
					Back
				</a>
				<a href="?page=outspokane_edit_entry&form=food_truck&id=<?php echo $entry->getId(); ?>" class="page-title-action">
					Edit
				</a>
				<a href="#" class="page-title-action" id="pride-delete-entry" data-form="food_truck" data-id="<?php echo $entry->getId(); ?>">
					Delete
				</a>
			</h1>
			...
			<div class="row">
				<div class="col-md-7">
					...
					<div class="well">
						...
						<table class="table">
							...
							<tr>
								<th>Event:</th>
								<td><?php echo $entry->getEntryYear(); ?> Food Truck</td>
							</tr>
					...
					{
						if ( preg_match('/\[pride_forms[^\]]*form=["\']food_truck["\'][^\]]*\]/', $page->post_content) === 1)
						{
				...
				<div class="col-md-5">
					...
					<div class="well" id="pride-forms-update-entry-notes" data-form="food_truck" data-id="<?php echo $entry->getId(); ?>">
					...
					<?php if ($entry->getTotal() > 0) { ?>

						<div class="well" id="pride-forms-mark-as-paid" data-form="food_truck" data-id="<?php echo $entry->getId(); ?>">
	...
	<?php } else { ?>
		...
		\OutSpokane\Entry::drawExportForm( \OutSpokane\FoodTruck::TABLE_NAME );
		$table = new \OutSpokane\EntryTable( \OutSpokane\FoodTruck::TABLE_NAME );
		$table->prepare_items();
		$table->display();
